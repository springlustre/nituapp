package com.springlustre.zhuazhua101.utils.http;
...
public class HttpClientUtil {
    ...
    static {
        ...
        defaultHeaders[3] = new BasicHeader("Content", "text/html,charset=utf-8");
        defaultHeaders[4] = new BasicHeader("Connection", "keep-alive");
        defaultHeaders[5] = new BasicHeader("Keep-Alive", "115");
//    defaultHeaders[6] = new BasicHeader("Host", "live.wallstreetcn.com");
//    defaultHeaders[7] = new BasicHeader("Referer", "http://live.wallstreetcn.com/");
    }
...
//        .setConnectionRequestTimeout(10000)
        ...
        headers.add(new BasicHeader("User-Agent", HttpContants.defaultUserAgent));
        headers.add(new BasicHeader("Accept", HttpContants.defaultAccept));
        headers.add(new BasicHeader("Accept-Encoding", HttpContants.defaultAcceptEncoding));
        headers.add(new BasicHeader("Accept-Language", HttpContants.defaultAcceptLanguage));
...
//                })
    ...
    public static CloseableHttpResponse executeHttpPost(String url, CloseableHttpClient client, Map<String, String> map) throws IOException {
        HttpPost post = new HttpPost(url);

        List<NameValuePair> paramters = new ArrayList<NameValuePair>();
        if (map != null) {
            for (Map.Entry<String, String> entry : map.entrySet()) {
                paramters.add(new BasicNameValuePair(entry.getKey(), entry.getValue()));
            }
        }
